...
1. Create the model.
2. View the results of the model.
...
## Bonus round: Multiple regression

Next week we will learn about multiple regression. However, if you are interested, you can try it out now.

We can add more than one predictor variable to the model. This is called multiple regression. Imagine we had a hypothesis that **Treatment type** and **Depression** both predict **Avoidance**. We could test this hypothesis by adding both variables to the model.

The code would look like this:

```{r eval=FALSE}

model2 <- lm(avoidance ~ treatment + depression, data = practical3_data) #<1>
...
Create a multiple regression model with **Treatment type** and **Depression** as predictors of **Avoidance**. 
