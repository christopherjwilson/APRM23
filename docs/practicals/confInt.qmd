...
We should already know the difference between the means (of participants over 18), from the code you ran earlier.
...
The difference between the means of the two groups (over 18s) is `r fitb(mean_diff)`
...
1. This code uses the <func>filter()</func> function to select only the rows of the dataset where the **treatment_group** variable is equal to "Treatment".
2. This code uses the <func>var()</func> function to calculate the variance of the **outcome** variable. The <func>pull()</func> function is used to extract the **outcome** variable from the dataset, so we can calculate the variance.
3. This code uses the <func>nrow()</func> function to calculate the number of rows in the dataset.
...
The upper confidence interval is `r fitb(mean_diff + (1.96 * se_diff) |> round(2))`.
